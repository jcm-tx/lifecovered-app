...
export default function PortalFamilyPage() {
  ...
  async function loadData() {
    ...
    const data = await res.json() as { children: Child[]; village: VillageMember[]; emails: UserEmail[] }
  ...
  async function saveEmail() {
    ...
    const res = await fetch('/api/portal/family', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'email', email: trimmed }),
    })
    if (res.status === 409) {
      ...
      <div style={styles.content}>
        ...
        <div style={styles.section}>
          <div style={styles.sectionHeader}>
            ...
            <button onClick={() => { setAddingEmail(true); setEmailError('') }} style={styles.addBtn}>+ Add</button>
          </div>
          <p style={styles.sectionNote}>Forward school emails and calendars from any of these addresses to schedule@lifecovered.app and Mary will add the events automatically.</p>
          ...
          {addingEmail && (
            <div style={styles.formCard}>
              ...
              <div style={styles.formActions}>
                <button onClick={saveEmail} style={styles.saveBtn} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button>
                <button onClick={() => { setAddingEmail(false); setNewEmail(''); setEmailError('') }} style={styles.cancelBtn}>Cancel</button>
              </div>
            </div>
          )}

          {emails.length === 0 && !addingEmail && (
            <div style={styles.empty}>No email addresses registered yet. <button onClick={() => setAddingEmail(true)} style={styles.inlineBtn}>Add one</button></div>
          )}

          {emails.map(item => (
            <div key={item.id} style={styles.personCard}>
              <div style={styles.personInfo}>
                <div style={styles.personName}>{item.email}</div>
              </div>
              <div style={styles.personActions}>
                <button onClick={() => deleteEmail(item.id)} style={styles.deleteBtn}>Remove</button>
              </div>
            </div>
          ))}
        </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  sectionNote: { fontSize: '13px', color: '#78716C', marginBottom: '16px', lineHeight: '1.5' },
  ...
  errorMsg: { fontSize: '13px', color: '#DC2626', marginTop: '8px' },
