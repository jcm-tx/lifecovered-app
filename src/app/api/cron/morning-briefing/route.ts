...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (families.length === 0) {
      return new NextResponse('No families found', { status: 200 })
    }
...
async function generateBriefing({
  ...
  const todayList = todayEvents.length > 0
    ? todayEvents.map(e => {
        ...
        const assignedRaw = Array.isArray(e.assigned_user) ? e.assigned_user[0] : e.assigned_user
        return `${e.title}${e.event_time ? ' at ' + e.event_time : ''}${childRaw?.name ? ' for ' + childRaw.name : ''}${assignedRaw?.name ? ' (assigned to ' + assignedRaw.name + ')' : ''}`
      }).join(', ')
    : 'nothing scheduled'
